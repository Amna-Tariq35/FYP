...
type MakeupBagContextValue = {
  items: MakeupBagItem[];
  keys: string[];
  bagId: string | null;
  loading: boolean;
  isInBag: (productKey: string, shadeKey?: string | null) => boolean;
  addItem: (
    productKey: string,
    shadeKey?: string | null,
    source?: MakeupBagSource,
  ) => Promise<{ ok: boolean; already?: boolean; reason?: string }>;
  removeItem: (productKey: string, shadeKey?: string | null) => Promise<void>;
  toggleItem: (
    productKey: string,
    shadeKey?: string | null,
    source?: MakeupBagSource,
  ) => Promise<{ inBag: boolean }>;
  count: number;
  isSignedIn: boolean;
};

const MakeupBagContext = createContext<MakeupBagContextValue | null>(null);
...
export function MakeupBagProvider({ children }: { children: React.ReactNode }) {
  ...
  // Fires once, app-wide — as soon as `status` flips to "signed_in".
  useEffect(() => {
        ...
        (payload) => {
          ...
          setItems((prev) => {
            ...
            if (payload.eventType === "DELETE") {
              next = prev.filter(
                (i) => bagItemKey(i.product_key, i.shade_key) !== key && i.id !== row.id,
              );
            } else {
              const exists = prev.some((i) => bagItemKey(i.product_key, i.shade_key) === key);
              ...
              if (exists) {
                next = prev.map((i) => (bagItemKey(i.product_key, i.shade_key) === key ? mapped : i));
              } else {
                next = [mapped, ...prev];
              }
            }
  ...
  const addItem = useCallback(
    async (productKey: string, shadeKey?: string | null, source: MakeupBagSource = "manual") => {
      const {
        data: { user },
      } = await supabase.auth.getUser();
      ...
      setItems((prev) => {
        const next = [
          { product_key: productKey, shade_key: sk, source, added_at: new Date().toISOString() },
          ...prev,
        ];
        const nextKeys = next.map((i) => bagItemKey(i.product_key, i.shade_key));
        setKeys(nextKeys);
  ...
  const value: MakeupBagContextValue = {
    items,
    keys,
    bagId,
    loading,
  ...
  return <MakeupBagContext.Provider value={value}>{children}</MakeupBagContext.Provider>;
}

export function useMakeupBag() {
  const ctx = useContext(MakeupBagContext);
  if (!ctx) {
    throw new Error("useMakeupBag must be used within a MakeupBagProvider");
  }
  return ctx;
}